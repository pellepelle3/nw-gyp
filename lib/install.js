...
function install (gyp, argv, callback) {
  ...
  var distUrl = gyp.opts['dist-url'] || gyp.opts.disturl || 'http://dl.nwjs.io'
  ...
  var isV13 = semver.gt(versionStr, '0.13.0-alpha0')
    ...
    mkdir(devDir, function (err, created) {
      ...
      function downloadNodeLib (done) {
        ...
        var dir32 = path.resolve(devDir, 'ia32')
          ...
          , nwLibPath32 = path.resolve(dir32, 'nw.lib')
          , nwLibPath64 = path.resolve(dir64, 'nw.lib')
          , nwLibUrl32 = distUrl + '/v' + version + '/nw.lib'
          , nwLibUrl64 = distUrl + '/v' + version + '/x64/nw.lib'

        log.verbose('32-bit nw.lib dir', dir32)
        log.verbose('64-bit nw.lib dir', dir64)
        log.verbose('`nw.lib` 32-bit url', nwLibUrl32)
        log.verbose('`nw.lib` 64-bit url', nwLibUrl64)
        ...
        mkdir(dir32, function (err) {
          ...
          log.verbose('streaming 32-bit nw.lib to:', nwLibPath32)

          var req = download(nwLibUrl32)
          if (!req) return
          req.on('error', done)
          req.on('response', function (res) {
            if (res.statusCode !== 200) {
            ...
            var ws = fs.createWriteStream(nwLibPath32)
            ws.on('error', cb)
            req.pipe(ws)
          })
          req.on('end', function () {
        ...
        if (!isLegacy) {
          mkdir(dir64, function (err) {
            if (err) return done(err)
            log.verbose('streaming 64-bit nw.lib to:', nwLibPath64)

            var req = download(nwLibUrl64)
            if (!req) return
            req.on('error', done)
            req.on('response', function (res) {
              if (res.statusCode !== 200) {
              ...
              var ws = fs.createWriteStream(nwLibPath64)
              ws.on('error', cb)
              req.pipe(ws)
            })
            req.on('end', function () {
        ...
        if (isV13) {
          ...
          var req2 = download(nodeLibUrl64)
          if (!req2) return
          req2.on('error', done)
